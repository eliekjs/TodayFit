...
import { alpineOwnedPickMany } from "./alpineOwnedSelection";

export function createAlpineSportMainHandles(deps: SportMainSelectorDeps): SportMainHandles {
  return {
    ...
    selectStrengthMainLifts(ctx: AlpineStrengthMainLiftContext): Exercise[] {
      ...
      traceNotes?.push("alpine_sport_owned:strength contract=" + contract.sessionType);
      ...
      if (primaryIntent && intentSlugs.length > 0) {
        ...
        if (primaryMatches.length > 0) {
          ...
          if (remainingCount > 0 && mainLifts[0]) {
            ...
            const rest = alpineOwnedPickMany(
              remainingPool,
              remainingCount,
              ctx.input,
              ctx.recentIds,
      ...
      if (mainLifts.length > 0) {
        applyAlpineUpstreamMainLiftsCoverage(mainLifts, ctx.replacementCatalog, "main_strength");
        traceNotes?.push("alpine_sport_owned:strength coverage_pass(main)");
      }
    ...
    applyStrengthAccessoryCoverage(ctx: AlpineStrengthAccessoryCoverageContext): void {
      applyAlpineUpstreamAccessoryPairsCoverage(ctx.mainLifts, ctx.pairs, ctx.replacementCatalog, "accessory");
    },
    ...
    selectHypertrophyVolume(ctx: AlpineHypertrophyVolumeContext): Exercise[] {
      ...
      traceNotes?.push("alpine_sport_owned:hypertrophy contract=" + contract.sessionType);
      ...
      if (dominantSlug && isHypertrophyPrimary && directSubFocusSlugs.length > 0 && wantCount > 0) {
        ...
        if (directDominantPool.length > 0) {
          ...
          const firstPick = alpineOwnedPickMany(
            directDominantPool,
            firstCount,
            ctx.input,
            ctx.recentIds,
      ...
      if (chosen.length > 0) {
        applyAlpineUpstreamMainLiftsCoverage(chosen, ctx.replacementCatalog, "main_hypertrophy");
        traceNotes?.push("alpine_sport_owned:hypertrophy coverage_pass(volume)");
      }
    ...
    refineHypertrophyPairsCoverage(chosen: Exercise[], pairs: Exercise[][], replacementCatalog: Exercise[]): void {
      applyAlpineUpstreamAccessoryPairsCoverage([], pairs, replacementCatalog, "main_hypertrophy");
    },
  };
}
