...
import {
  ...
  selectCooldownExercisesWithFallback,
  buildCooldownStretchPool,
  getGeneralCooldownFallbackTargets,
...
import {
  ...
  shouldAllocatePrehabAccessorySlots,
} from "./intentSlotAllocator";
import {
  exerciseIsLowerBodyDominantPowerMovement,
  exerciseIsUpperBodyPowerMovement,
...
import { resolveSessionFeelProfile, isImpliedSportOnlySession } from "./sessionFeelProfile";
import {
...
import {
  ...
  getActiveBlockFillInput,
  isAccessoryEligible,
  isAssessmentExercise,
  isConditioningEligible,
  isConditioningBlockPoolCandidate,
...
export function filterByConstraintsForPool(
  ...
  return exercises.filter((e) => {
    ...
    const upperOnly =
      allowedFamilies!.every((family) => family === "upper_push" || family === "upper_pull");
    const lowerOnly = allowedFamilies!.every((family) => family === "lower_body");
    if (upperOnly && exerciseIsLowerBodyDominantPowerMovement(e)) return false;
    if (lowerOnly && exerciseIsUpperBodyPowerMovement(e)) return false;
    if (e.modality === "conditioning") {
      // Generic cardio stays available, but clearly regional dynamic work was
      // rejected above. Loaded conditioning moves also honor Muscle-day gates.
      if (lowerOnly && exerciseIsLowerBodyDominantPowerMovement(e)) return true;
      const muscleGate =
...
function isSandbagLoadVariant(e: Exercise): boolean {
  const id = (e.id ?? "").toLowerCase();
  const name = (e.name ?? "").toLowerCase();
  return (
    (id.includes("sandbag") && id.includes("load")) ||
    (name.includes("sandbag") && name.includes("load"))
  );
}
...
const PSEUDO_EXERCISE_ENTITY_SLUGS = new Set([
  "glutes",
  "glute",
  "quads",
  "hamstrings",
...
function isPseudoExerciseEntity(e: Exercise): boolean {
  const id = (e.id ?? "").toLowerCase().trim().replace(/\s+/g, "_");
  const name = (e.name ?? "").toLowerCase().trim().replace(/\s+/g, "_");
  return PSEUDO_EXERCISE_ENTITY_SLUGS.has(id) || PSEUDO_EXERCISE_ENTITY_SLUGS.has(name);
}
...
export function getHardConstraintRejectReason(
  ...
  if (isPseudoExerciseEntity(e)) return "pseudo_exercise_entity";
...
export function scoreExercise(
  ...
  // Sub-goal replacement behavior:
  // If user selected sub-focuses for the active primary goal, treat those as the dominant intent.
  // Exercises that do not match any selected primary sub-focus get a meaningful down-rank so
  // the parent goal acts as fallback only when sub-focus-constrained candidates are scarce.
  const primarySubFocusSlugs = getRankedGoalSubFocusSlugsForGoal(input, primary);
  if (primarySubFocusSlugs.length > 0) {
    const primaryGoalKeys = goalSubFocusKeysForPrimary(primary);
    const hasPrimarySubFocusMatch = primaryGoalKeys.some((goalKey) =>
      primarySubFocusSlugs.some((slug) =>
        exerciseMatchesGoalSubFocusSlugUnified(exercise, goalKey, slug)
      )
    );
    if (hasPrimarySubFocusMatch) {
      applyToTotal(6, "generic");
    } else {
      applyToTotal(-6, "generic");
    }
  }

  // Calisthenics primary uses `goal_sub_focus.calisthenics` (see GOAL_SUB_FOCUS_OPTIONS). Legacy sessions may still have subs under `strength`.
  // Barbell strength primary uses `goal_sub_focus.strength` only.
  const strengthKeyedSubFocus =
    primary === "calisthenics"
      ? goalSubFocus?.calisthenics ?? goalSubFocus?.strength
      : primary === "strength"
        ? goalSubFocus?.strength
        : undefined;
  if (
    ...
    const weightsArr =
      primary === "calisthenics"
        ? (input.goal_sub_focus_weights?.calisthenics ??
            input.goal_sub_focus_weights?.strength ??
            ranked.map(() => 1 / (ranked.length || 1)))
  ...
  if (inputHasVerticalJumpSubFocus(input)) {
    const blockNorm = blockTypeNorm;
    if (
      (blockNorm === "main_strength" || blockNorm === "main_hypertrophy") &&
      !exerciseIsMedBallPowerThrow(exercise)
    ) {
      const attrs = getExerciseTagSlugs(exercise);
      if (attrs.has("squat_pattern") || attrs.has("hinge_pattern") || attrs.has("single_leg_strength")) {
        applyToTotal(2, "generic");
      }
    }
  }

  const activeSubSlugs = collectActiveSubFocusSlugs(input);
  if (activeSubSlugs.length > 0) {
    const registryScore = aggregateSubFocusSelectionScore(exercise, activeSubSlugs);
    if (registryScore !== 0) {
      applyToTotal(registryScore, "generic");
    }
  }
...
function getPrescription(
  ...
  const rxCtx = sessionInput
    ? resolvePrescriptionContext(blockType, exercise, sessionInput, isAccessory)
    : {
        blockType,
        primaryGoal: (primaryGoal ?? "hypertrophy") as PrimaryGoal,
        isAccessory: isAccessory ?? false,
      };
  blockType = rxCtx.blockType;
  const goal = rxCtx.primaryGoal;
...
function isSelectableInBlock(
  exerciseId: string,
  chosen: Exercise[],
  sessionUsedIds?: Set<string>
): boolean {
  return isExerciseAvailableForSession(exerciseId, sessionUsedIds ?? new Set(), chosen.map((c) => c.id));
}
...
function selectExercisesSportPatternIterative(
  ...
  const sessionUsedIds = opts.sessionUsedIds;
  ...
  for (let round = 0; chosen.length < count && round < Math.max(pool.length * 4, 24); round++) {
    let remaining = pool.filter((e) => isSelectableInBlock(e.id, chosen, sessionUsedIds));
    ...
    for (let i = 0; i < Math.max(100, randomPool.length * 5) && !picked; i++) {
      ...
      if (!item || !isSelectableInBlock(item.exercise.id, chosen, sessionUsedIds)) continue;
...
function selectExercises(
  ...
  selectionOptions?: {
    ...
    targetCardioExerciseShare?: number;
    intent_survival?: IntentSurvivalSelectionOpts;
    /** Session ids already selected in prior blocks — enforces max-one redundancy families. */
    sessionUsedIds?: Set<string>;
  }
): { exercises: Exercise[] } {
  const opts = selectionOptions ?? {};
  const sessionUsedIds = opts.sessionUsedIds;
  const blockNorm = (opts.blockType ?? "").toLowerCase().replace(/\s/g, "_");
  const fillConstraints = getActiveBlockFillConstraints();
  const fillInput = getActiveBlockFillInput();
  const verticalJumpIntent = inputHasVerticalJumpSubFocus(input);
  let poolForSelection = pool;
  if (verticalJumpIntent && blockNorm !== "warmup" && blockNorm !== "cooldown" && blockNorm !== "mobility" && blockNorm !== "recovery") {
    poolForSelection = pool.filter((e) => exerciseEligibleForVerticalJumpSession(e, true));
  }
  if (
    fillConstraints &&
    blockNorm !== "warmup" &&
    ...
    poolForSelection = poolForSelection.filter((e) =>
  ...
  const scored = poolForSelection.map((e) => ({
    exercise: e,
    ...scoreExercise(e, input, recentIds, movementCounts, fatigueState, scoreOpts),
  }));
  scored.sort((a, b) => b.score - a.score);
  ...
  const tierBand =
    ...
    opts.blockType === "power"
      ? 5.5
      : 2.5;
  // Activation: ontology + target-match bonuses can stack ~4–8pts ahead of otherwise-good prep moves,
  ...
  for (let k = 0; k < needCategories && chosen.length < count; k++) {
    ...
    let candidates = topOverall.filter(
      (x) =>
        ...
        isSelectableInBlock(x.exercise.id, chosen, sessionUsedIds) &&
  ...
  for (let i = 0; chosen.length < count && i < topOverall.length * 2; i++) {
    const item =
      opts.blockType === "accessory" && hasSportIntent(input)
        ? pickStratifiedAccessoryCandidate(randomPool, chosen, rng, sessionUsedIds)
  ...
  for (const { exercise } of fillOrdered) {
    ...
    if (!isSelectableInBlock(exercise.id, chosen, sessionUsedIds)) continue;
    const nextCount = (movementCounts.get(exercise.movement_pattern) ?? 0) + 1;
    if (nextCount > patternCap) continue;
    if (wouldBeThreeSameClusterInARow(chosen, exercise)) continue;
    chosen.push(exercise);
  ...
  if (isv) {
    ...
    isv.collector.pushSelectionPass({
      ...
      chosen_why: finalChosen.map(
        (_, i) =>
          `pick_${i + 1}: random_within_top_score_tier (band=${effectiveTierBand}); best_in_view≈${bestScore.toFixed(3)}`
      ),
    });
  }
...
function buildWarmup(
  ...
  if (speedPowerTemplate.warmupDecelPrep) {
    const codPrepPool = [...exercises, ...fallbackPool].filter((e) => {
      ...
      return warmupCodPrepSelectionScore(e) > 0;
    });
    if (codPrepPool.length > 0) {
      const seen = new Set(pool.map((e) => e.id));
      pool = [...pool, ...codPrepPool.filter((e) => !seen.has(e.id))];
    }
  }
  ...
  const focusRelevantPool =
    preferredWarmupTargets.length > 0
      ...
      : equipmentPool;
  let finalPool = focusRelevantPool.length > 0 ? focusRelevantPool : equipmentPool;
  /** When ontology target overlap yields a thin list, scoring has few competitors; bulk up with warmup-eligible moves (selection still boosts matches via prefs). */
  const MIN_ACTIVATION_CANDIDATE_POOL = 24;
  if (finalPool.length > 0 && finalPool.length < MIN_ACTIVATION_CANDIDATE_POOL) {
    const seen = new Set(finalPool.map((e) => e.id));
    const merged = [...finalPool];
    for (const e of equipmentPool) {
      if (merged.length >= MIN_ACTIVATION_CANDIDATE_POOL) break;
      if (!seen.has(e.id)) {
        seen.add(e.id);
        merged.push(e);
      }
...
function buildCooldown(
  ...
  const policyMin = options.minCooldownItems ?? 0;
  const constraintMin = options.constraints.min_cooldown_mobility_exercises ?? 0;
  const minMobility = Math.max(policyMin, constraintMin);
  const requiresCooldown = options.requiresCooldown === true || minMobility > 0;
  const useOntologyCooldown =
    requiresCooldown || minMobility > 0 || options.mainWorkFamilies.length > 0;
  const recoveryEmphasis =
    ...
    ) || isRecoveryMobilityPrimaryGoal(input.primary_goal);
  const preferredTargets = [
    ...new Set([
      ...getPreferredCooldownTargetsFromFamilies(options.mainWorkFamilies),
      ...(options.preferredTargets ?? []),
      ...getGeneralCooldownFallbackTargets(input.focus_body_parts ?? []),
    ]),
  ];
  ...
  const fallbackEquipmentOk = (options.fallbackPool ?? exercises).filter((e) =>
    isCooldownEligibleEquipment(e.equipment_required ?? [])
  );
  const cooldownPool = buildCooldownStretchPool(equipmentOk, {
    alreadyUsedIds: used,
    recoveryEmphasis,
  });
  const guaranteeCooldownPool = buildCooldownStretchPool(fallbackEquipmentOk, {
    alreadyUsedIds: used,
    recoveryEmphasis,
  });
  ...
  if (useOntologyCooldown) {
    const defaultCount = input.duration_minutes <= 30 ? 2 : 3;
    const effectiveMin = Math.max(minMobility, requiresCooldown ? defaultCount : 0, 1);
    const maxItems = recoveryEmphasis
      ? (input.duration_minutes <= 30 ? Math.max(effectiveMin, 4) : Math.max(effectiveMin, 6))
      : (input.duration_minutes <= 30 ? Math.max(effectiveMin, 3) : Math.max(effectiveMin, 4));
    chosen = selectCooldownExercisesWithFallback(cooldownPool, guaranteeCooldownPool, {
      minMobilityCount: effectiveMin,
      preferredTargets,
      alreadyUsedIds: used,
      rng,
      maxItems,
    ...
    if (chosen.length < effectiveMin) {
      const pool = guaranteeCooldownPool.filter((e) => isExerciseAvailableForSession(e.id, used));
      const ids = shuffleWithSeed([...pool], rng);
      for (const e of ids) {
        if (chosen.length >= effectiveMin) break;
        chosen.push(e);
        used.add(e.id);
      }
    }
  } else {
    const pool = cooldownPool.filter((e) => isExerciseAvailableForSession(e.id, used));
    const count =
    ...
    if (chosen.length < count) {
      const fallback = guaranteeCooldownPool.filter((e) => isExerciseAvailableForSession(e.id, used));
      const fallbackIds = shuffleWithSeed([...fallback], rng);
      for (const e of fallbackIds) {
        if (chosen.length >= count) break;
        chosen.push(e);
        used.add(e.id);
      }
  ...
  if (recoveryEmphasis && chosen.length > 0 && isExerciseAvailableForSession("breathing_cooldown", used)) {
...
// --- Goal-dedicated main blocks (≥2 sub-focuses; general sessions only — no sport-pattern transfer) ---
const GOAL_DEDICATED_SWAP_POOL_CAP = 200;
const GOAL_DEDICATED_SCORE_SHORTLIST = 12;

type GoalDedicatedSubFocusEntry = { slug: string; goalKey: string; weight: number };

function hasSportPatternTransferForGoalDedicated(input: GenerateWorkoutInput): boolean {
  return (
    ...
    (resolveSnowSportKind(input) != null && snowSportBodyFocusAllows(input))
  );
}

/** Speed/COD/RSA sport sub-focuses use intent-dedicated power blocks instead of pattern-transfer main_strength. */
function sportTransferYieldToSpeedPowerIntentSlots(input: GenerateWorkoutInput): boolean {
  const leaves = deriveLeafEntries(input.session_intent?.ranked_intent_entries ?? []);
  const powerLeaves = leaves.filter(
    (e) => e.kind === "sport_sub_focus" && isPowerStyleSportIntentEntry(e)
  );
  return powerLeaves.length >= 1;
}

function getPrimarySubFocusWeightedEntriesDedicated(
  input: GenerateWorkoutInput,
  primary: PrimaryGoal
): GoalDedicatedSubFocusEntry[] {
  const gsf = input.goal_sub_focus ?? {};
  const gw = input.goal_sub_focus_weights ?? {};
  const keys = goalSubFocusKeysForPrimary(primary);
  const raw: GoalDedicatedSubFocusEntry[] = [];
  for (const goalKey of keys) {
    const slugs = gsf[goalKey];
    if (!slugs?.length) continue;
    const weights = gw[goalKey] ?? slugs.map(() => 1 / slugs.length);
    for (let i = 0; i < slugs.length; i++) {
      raw.push({ slug: slugs[i]!, goalKey, weight: weights[i] ?? 0 });
    }
  }
  const bySlug = new Map<string, GoalDedicatedSubFocusEntry>();
  for (const e of raw) {
    const prev = bySlug.get(e.slug);
    if (!prev || e.weight > prev.weight) bySlug.set(e.slug, e);
  }
  return [...bySlug.values()].sort((a, b) => b.weight - a.weight);
}

/** Multiple sub-focuses with distinct sections + swap pools; not used for calisthenics or sport-transfer sessions. */
export function shouldUseGoalDedicatedMainBlocks(input: GenerateWorkoutInput): boolean {
  if (hasSportPatternTransferForGoalDedicated(input)) return false;
  const primary = input.primary_goal;
  if (primary !== "strength" && primary !== "hypertrophy" && primary !== "body_recomp") return false;
  const entries = getPrimarySubFocusWeightedEntriesDedicated(input, primary);
  const meaningful = entries.filter((e) => e.slug !== "balanced");
...
function exerciseMatchesGoalDedicatedSubFocus(
  ...
  if (primary === "hypertrophy") {
    return exerciseMatchesHypertrophySubFocusSlug(ex, slug);
  }
  if (primary === "body_recomp") {
    return (
      exerciseMatchesHypertrophySubFocusSlug(ex, slug) ||
      exerciseMatchesGoalSubFocusSlugUnified(ex, goalKey, slug)
    );
  }
  return exerciseMatchesGoalSubFocusSlugUnified(ex, goalKey, slug);
}

function humanizeGoalDedicatedSlug(slug: string): string {
  return slug.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}

function capSwapPoolIds(ids: string[], cap: number): string[] {
  return [...new Set(ids)].sort().slice(0, cap);
}

function pickFromScoredShortlistRandom(
  pool: Exercise[],
  input: GenerateWorkoutInput,
  recentIds: Set<string>,
  movementCounts: Map<string, number>,
  ...
  scored.sort((a, b) => b.score - a.score);
  const top = scored.slice(0, Math.min(GOAL_DEDICATED_SCORE_SHORTLIST, scored.length)).map((x) => x.ex);
  const arr = [...top];
  ...
  for (const ex of arr) {
    ...
    if (!isSelectableInBlock(ex.id, picked, opts.sessionUsedIds)) continue;
    picked.push(ex);
    movementCounts.set(ex.movement_pattern, (movementCounts.get(ex.movement_pattern) ?? 0) + 1);
    if (opts.sessionFatigueRegions) addExerciseFatigueRegionsToSession(opts.sessionFatigueRegions, ex);
  }
  if (picked.length < count) {
    const fromSelect = selectExercises(
      workPool.filter((ex) => isSelectableInBlock(ex.id, picked, opts.sessionUsedIds)),
      input,
      recentIds,
      movementCounts,
      count - picked.length,
      rng,
      fatigueState,
      { ...opts, sessionUsedIds: opts.sessionUsedIds }
    ).exercises;
    for (const ex of fromSelect) {
      if (picked.length >= count) break;
      if (!isSelectableInBlock(ex.id, picked, opts.sessionUsedIds)) continue;
      picked.push(ex);
    }
...
function shouldUseIntentSlotAllocation(input: GenerateWorkoutInput): boolean {
  if (hasSportPatternTransferForGoalDedicated(input) && !sportTransferYieldToSpeedPowerIntentSlots(input)) {
    return false;
  }
  const entries = input.session_intent?.ranked_intent_entries ?? [];
  if (entries.length >= 2) return true;
  const leaves = deriveLeafEntries(entries);
  return leaves.length === 1 && isPowerStyleSportIntentEntry(leaves[0]!);
}
...
function tryBuildIntentSlotAllocatedMainInject(
  ...
  /** Broader pool when sport-profile filtering empties a power sub-focus slot. */
  broadPool?: Exercise[]
  ...
  const leaves = deriveLeafEntries(input.session_intent?.ranked_intent_entries ?? []).filter(
    (leaf) => leaf.weight > 0.001
  );
  if (leaves.length < 2) {
    if (leaves.length !== 1 || !isPowerStyleSportIntentEntry(leaves[0]!)) return undefined;
  }

  const powerSportLeafCount = leaves.filter(
    (leaf) => leaf.kind === "sport_sub_focus" && isPowerStyleSportIntentEntry(leaf)
  ).length;
  ...
  const targetMainByDuration = targetMainCompoundCount(input.duration_minutes);
  ...
  const allocMain = rebalanceAllocMinOneSlotPerPowerLeaf(
    leaves,
    allocateSlotsBySubFocusWeights(leafKeys, leafWeights, mainLiftCount)
  );
  ...
  for (let i = 0; i < leaves.length; i++) {
    ...
    const nMainCapped =
      entryBlockType === "power"
        ? Math.max(
            powerSportLeafCount >= 2 ? 1 : 0,
            Math.min(nMain, maxPowerExercisesForDuration(input.duration_minutes ?? 45))
          )
        : nMain;
    if (nMainCapped <= 0) continue;
    ...
    let pool = searchExercises.filter(
      (e) =>
        isExerciseAvailableForSession(e.id, used) &&
    ...
    if (pool.length < nMainCapped && entryBlockType === "power" && entry.kind === "sport_sub_focus") {
      const fallback = searchExercises.filter(
        (e) =>
          isExerciseAvailableForSession(e.id, used) &&
          matchesIntentEntry(e, entry) &&
    ...
    if (
      pool.length < nMainCapped &&
      entryBlockType === "power" &&
      broadPool &&
      entry.kind === "sport_sub_focus"
    ) {
      const fallback = broadPool.filter(
        (e) =>
          isExerciseAvailableForSession(e.id, used) &&
          matchesIntentEntry(e, entry) &&
          isMainWorkCandidateForIntentEntry(e, entry, entryPrimary, input.focus_body_parts) &&
...
const BLENDED_MANUAL_STRENGTH_SUB_FOCUS_SLUGS = new Set(["squat", "deadlift_hinge", "full_body"]);

function hasManualSquatHingeSubFocus(input: GenerateWorkoutInput): boolean {
  const slugs = [
    ...(input.goal_sub_focus?.strength ?? []),
    ...(input.session_intent?.goal_sub_focus_by_goal?.strength ?? []),
  ].map((s) => s.toLowerCase().replace(/\s/g, "_"));
  return slugs.some((s) => BLENDED_MANUAL_STRENGTH_SUB_FOCUS_SLUGS.has(s));
}

function shouldReserveBlendedManualStrengthBlock(input: GenerateWorkoutInput): boolean {
  const sw = Math.max(0, Math.min(1, input.sport_weight ?? input.session_intent?.sport_weight ?? 0));
  if (sw >= 0.85 || sw <= 0) return false;
  if ((input.sport_slugs?.length ?? 0) === 0) return false;
  return hasManualSquatHingeSubFocus(input);
}

/** Sport + manual blend: reserve visible squat/hinge main_strength before sport power slots dominate. */
function tryBuildBlendedManualStrengthReserve(
  exercises: Exercise[],
  input: GenerateWorkoutInput,
  used: Set<string>,
  recentIds: Set<string>,
  ...
  if (!shouldReserveBlendedManualStrengthBlock(input)) return undefined;
  if (existingBlocks.some((b) => b.block_type === "main_strength")) return undefined;

  const selectedSlugs = [
    ...(input.goal_sub_focus?.strength ?? []),
  ]
    .map((s) => s.toLowerCase().replace(/\s/g, "_"))
    .filter((s) => BLENDED_MANUAL_STRENGTH_SUB_FOCUS_SLUGS.has(s));
  const targetSlugs =
    selectedSlugs.length > 0
      ? [...new Set(selectedSlugs.filter((s) => s === "squat" || s === "deadlift_hinge"))]
  ...
  const picked: Exercise[] = [];
  for (const slug of targetSlugs.slice(0, 2)) {
    const pool = mainPool.filter(
      (e) =>
        isExerciseAvailableForSession(e.id, used, picked.map((p) => p.id)) &&
        exerciseMatchesGoalSubFocusSlugUnified(e, "strength", slug)
    );
    if (pool.length === 0) continue;
    const slotRng = rngForWorkoutSlot(input.seed ?? 0, "blended_manual_strength", slug);
    const one = pickFromScoredShortlistRandom(
      pool,
      input,
      recentIds,
      movementCounts,
  ...
  for (const ex of picked) {
    ...
    items.push({
      ...
      rest_seconds: p.rest_seconds,
      coaching_cues: p.coaching_cues,
      reasoning_tags: ["main_lift", "strength", "blended_manual_strength", ...(ex.tags.goal_tags ?? [])],
      unilateral: ex.unilateral ?? false,
    });
  }
  ...
  return [
    {
      ...
      format: "straight_sets",
      title: "Main strength",
      reasoning: "Manual squat/hinge sub-focus reserved before sport power work in this blended session.",
      items,
      estimated_minutes: Math.min(Math.max(est, 4), Math.ceil((input.duration_minutes ?? 45) * 0.35)),
    },
  ];
}
...
function buildUnifiedIntentSpecialtyBlocks(
  ...
  const leaves = deriveLeafEntries(input.session_intent?.ranked_intent_entries ?? []);
  if (leaves.length < 2) {
    if (leaves.length === 1 && isPowerStyleSportIntentEntry(leaves[0]!)) {
      const totalSlots = estimateIntentWorkingExerciseSlots(input.duration_minutes);
      return {
        specialtyBlocks: [],
        mainCompoundSlots: totalSlots,
        slotPlanSummary: [
          {
            slug: leaves[0]!.slug,
            archetype: "power",
  ...
  for (const planEntry of plan) {
    ...
    if (archetype === "conditioning") {
      ...
      const pool = searchPool.filter((e) => isExerciseAvailableForSession(e.id, used) && matchesIntentEntry(e, entry));
    ...
    if (archetype === "prehab") {
      if (!shouldAllocatePrehabAccessorySlots(input)) {
        slotPlanSummary.push({ slug: entry.slug, archetype, allocatedSlots: slots, pickedCount: 0, allMatched: true });
        continue;
      }
      const searchPool = broadPool ?? exercises;
...
export function buildSupersetIntentPreferenceScores(
  ...
  for (const ex of pool) {
    let s = 0;
    const exSlugs = getExerciseTagSlugs(ex);
    for (const [slug, w] of prefWeights) {
      if (exSlugs.has(slug)) s += w;
    }
    for (const t of ex.tags.goal_tags ?? []) {
      if (primaryTags.includes(t)) s += WEIGHT_PRIMARY_GOAL;
    }
    if (sportSlugs?.length) {
      const exerciseSportTags = new Set(
    ...
    if (stv && stv.size > 0) {
      ...
      if (eqWeights && Object.keys(eqWeights).length > 0) {
        s += alignmentScore(eqWeights, stv) * WEIGHT_QUALITY_ALIGNMENT;
      }
    }
    map.set(ex.id, s);
  }
...
function tryBuildGoalDedicatedMainStrengthInject(
  exercises: Exercise[],
  input: GenerateWorkoutInput,
  used: Set<string>,
  recentIds: Set<string>,
  ...
  const primary = input.primary_goal;
  if (primary !== "strength" || !shouldUseGoalDedicatedMainBlocks(input)) return undefined;
  ...
  if (input.energy_level === "low") compoundMin = Math.min(compoundMin, 1);
  const targetMainLiftCountByDuration = targetMainCompoundCount(input.duration_minutes);
  const mainLiftCount = Math.min(Math.max(compoundMin, targetMainLiftCountByDuration), mainPool.length);
  if (mainLiftCount < 2) return undefined;

  const slugs = entries.map((e) => e.slug);
  const weights = entries.map((e) => e.weight);
  const alloc = allocateSlotsBySubFocusWeights(slugs, weights, mainLiftCount);
  const wantsSupersets = input.style_prefs?.wants_supersets !== false;

  const baseSelectionOpts = {
  ...
  const outBlocks: WorkoutBlock[] = [];
  const strengthReason =
    "Compound lifts for strength — this section matches one of your ranked strength sub-focuses.";

  for (const entry of entries) {
    const n = alloc.get(entry.slug) ?? 0;
    if (n <= 0) continue;

    let poolSeg = mainPool.filter(
      (e) =>
        isExerciseAvailableForSession(e.id, used) &&
        exerciseMatchesGoalDedicatedSubFocus(e, primary, entry.goalKey, entry.slug)
    );
    if (poolSeg.length < n) return undefined;

    const swapPoolIds = capSwapPoolIds(
      mainPool
        .filter((e) => exerciseMatchesGoalDedicatedSubFocus(e, primary, entry.goalKey, entry.slug))
        .map((e) => e.id),
      GOAL_DEDICATED_SWAP_POOL_CAP
    );
    ...
    if (picked.length < n) return undefined;

    const goal_intent: WorkoutBlockGoalIntent = {
      goal_slug: primary,
      sub_focus_slug: entry.slug,
      goal_sub_focus_key: entry.goalKey,
      swap_pool_exercise_ids: swapPoolIds,
    };
    ...
    if (pairMain) {
      ...
      const restPerRoundSec = Math.max(pA.rest_seconds ?? 0, pB.rest_seconds ?? 0);
      const restForEstimateSec = Math.min(restPerRoundSec, 90);
      const workMinPerRound = 1.1;
      outBlocks.push({
        block_type: "main_strength",
        format: "superset",
        title: `Main strength (${label})`,
        reasoning: strengthReason,
        goal_intent,
        items: [itemA, itemB],
        supersetPairs: [[itemA, itemB]],
  ...
  return outBlocks.length > 0 ? { blocks: outBlocks } : undefined;
}

function tryBuildGoalDedicatedHypertrophyBlocks(
  exercises: Exercise[],
  input: GenerateWorkoutInput,
  used: Set<string>,
  recentIds: Set<string>,
  ...
  if (
    ...
    !shouldUseGoalDedicatedMainBlocks(input)
  ) {
    return undefined;
  }
  ...
  let pairCount = targetHypertrophyPairCount(input.duration_minutes);
  if (input.energy_level === "low") pairCount = Math.min(pairCount, 2);
  const wantCount = Math.min(pairCount * 2, durationScale * 2, maxExercises, pool.length);
  if (wantCount < 2) return undefined;

  const slugs = entries.map((e) => e.slug);
  const weights = entries.map((e) => e.weight);
  const alloc = allocateSlotsBySubFocusWeights(slugs, weights, wantCount);

  const baseSelectionOpts = {
    blockType: "main_hypertrophy" as const,
    sessionFatigueRegions,
    sessionMovementPatternCounts: movementCounts,
    sessionHasBilateralLowerBody: (movementCounts.get("squat") ?? 0) + (movementCounts.get("hinge") ?? 0) > 0,
    historyContext,
  ...
  for (const entry of entries) {
    ...
    if (chosen.length < n) return undefined;
    ...
    const dedicatedPairPrefs = buildSupersetIntentPreferenceScores(chosen, input);
    const rawPairs: Exercise[][] =
      targetPairCount > 0 && chosen.length >= 2
        ? (pickBestSupersetPairs(chosen, targetPairCount, new Set<string>(), undefined, dedicatedPairPrefs) as [Exercise, Exercise][])
        : [];
    let pairs: Exercise[][] = rawPairs;
    ...
    const format: BlockFormat =
      wantsSupersets && pairs.every((p) => p.length === 2) ? "superset" : "straight_sets";
    ...
    blocks.push({
      ...
      format,
      title: `Main hypertrophy (${label})`,
      reasoning: `Volume for ${label.toLowerCase()} — matches your ranked muscle-building sub-focus.`,
      goal_intent,
      items,
...
function buildMainStrength(
  ...
  goalDedicatedInject?: { blocks: WorkoutBlock[] } | null
): WorkoutBlock[] {
  ...
  if (goalDedicatedInject?.blocks.length) {
    const exerciseById = new Map(exercises.map((e) => [e.id, e]));
    for (const b of goalDedicatedInject.blocks) {
      blocks.push(b);
      for (const it of b.items) {
        used.add(it.exercise_id);
      }
    }
    mainLifts = goalDedicatedInject.blocks
      .flatMap((b) => b.items)
      .map((it) => exerciseById.get(it.exercise_id))
  ...
  if (sportHandles && snowMainRule && intentContractForSportMain && primarySportIsAlpineSkiing(input)) {
    ...
    mainLifts = sportHandles.selectStrengthMainLifts({
      ...
      sportPatCounts,
      alpineEmphasis,
      replacementCatalog: exercises.filter((e) => isExerciseAvailableForSession(e.id, used)),
    ...
    if (snowMainRule && snowSportApplies && snowKind && mainLifts.length > 0) {
      const alpineCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used));
    ...
    if (rockMainRule && rockClimbingPatternTransferApplies(input) && mainLifts.length > 0) {
      const rockCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used));
  ...
  if (isFullBodyStrengthSession && mainLifts.length > 1) {
    ...
    // Never displace the user's selected lift intent (e.g. Build Strength → Squat) when
    // rebalancing full-body upper/lower mains — that was replacing the intent anchor.
    const isProtectedIntentLift = (exercise: Exercise) =>
      !!primaryIntent && exerciseMatchesStrengthIntentStrong(exercise, primaryIntent);
    const replaceAt = (idx: number, preferredBucket?: "upper" | "lower"): boolean => {
      const current = adjusted[idx];
      if (!current || isProtectedIntentLift(current)) return false;
    ...
    // Bias toward an even upper/lower split for full-body strength, but never at the
    // expense of the selected strength sub-focus (squat / hinge / press / pull).
    ...
    while (Math.abs(upperCount - lowerCount) > 1) {
      if (upperCount > lowerCount) {
        const idx = adjusted.findIndex(
          (exercise) =>
            classifyUpperLowerBucket(exercise) === "upper" && !isProtectedIntentLift(exercise)
        );
        if (idx < 0 || !replaceAt(idx, "lower")) break;
      } else {
        const idx = adjusted.findIndex(
          (exercise) =>
            classifyUpperLowerBucket(exercise) === "lower" && !isProtectedIntentLift(exercise)
        );
        if (idx < 0 || !replaceAt(idx, "upper")) break;
      }
  ...
  const accessoryPairCountTarget = targetAccessoryPairCount(input.duration_minutes);
  ...
  if (blockStructure.suppressAccessoryBlocks) pairCount = 0;
  if (pairCount && wantsSupersets) {
    let available = accessoryPool.filter((e) => isExerciseAvailableForSession(e.id, used));
    ...
    if (sportHandles?.sportSlug === "rock_climbing" && rockAccessoryRule && pairs.length > 0 && mainLifts.length > 0) {
      ...
      const rockAccCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used));
      ...
      const alpineAccCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used));
      sportHandles.applyStrengthAccessoryCoverage({
        mainLifts,
        pairs: pairs as Exercise[][],
    ...
    if (items.length) {
      ...
      if (input.energy_level === "high" && pairCount === 1) {
        ...
        const coreCandidate = accessoryPool.find((e) => isExerciseAvailableForSession(e.id, used) && coreFamily(e));
        if (coreCandidate) {
...
function buildPowerBlock(
  ...
  const activeExplosiveSubSlugs =
    rankedPowerIntents.length > 0 && sessionAssemblesPowerBlock(input)
      ...
      : collectActiveSubFocusSlugs(input).filter(
          (s) =>
            isExplosivePlyometricSportSubFocusSlug(s) || s === "speed" || s === "change_of_direction"
        );
  ...
  let powerPool = exercises.filter((e) => {
    ...
    if (verticalJumpIntent || activeExplosiveSubSlugs.length > 0) {
      const subSlugs = verticalJumpIntent ? ["vertical_jump", ...activeExplosiveSubSlugs] : activeExplosiveSubSlugs;
  ...
  if (isUpperOnlyFocusBodyParts(input.focus_body_parts)) {
    const upperFiltered = powerPool.filter((e) => !exerciseIsLowerBodyDominantPowerMovement(e));
    if (upperFiltered.length > 0) powerPool = upperFiltered;
  }

  const wantCount = maxPowerExercisesForDuration(input.duration_minutes ?? 45);
...
function pickConditioningExercise(
  ...
  if (preferredSubFocusSlugs?.length) {
    ...
    if (directMatch.length > 0) {
      ...
      const wantsStrictZone2 = preferredSubFocusSlugs.some(
        (s) => s === "zone2_aerobic_base" || s === "zone2_long_steady" || s === "durability"
      );
      if (wantsStrictZone2) {
        const z2Strict = candidatePool.filter((e) => isTrueSteadyStateZone2Cardio(e));
        if (z2Strict.length > 0) candidatePool = z2Strict;
      }
    } else if (preferredSubFocusSlugs.some((slug) => slug === "hills")) {
      const hillFallback = pool.filter((e) => isHillBiasExercise(e));
      if (hillFallback.length > 0) candidatePool = hillFallback;
    }
  }
 ...
 */
function getCardioFinisherIntentSlugs(
  input: GenerateWorkoutInput,
  goalPriorityOrder: ("conditioning" | "endurance")[] = ["conditioning", "endurance"]
): string[] | undefined {
  const gsf = input.goal_sub_focus ?? {};
  const gw = input.goal_sub_focus_weights ?? {};
  const scored: { slug: string; w: number }[] = [];
  for (const goalSlug of goalPriorityOrder) {
...
function narrowCardioPoolByConditioningIntents(pool: Exercise[], intentSlugs: string[]): Exercise[] {
  ...
  if (top === "durability") {
    const strict = pool.filter(
      (e) => exerciseHasSubFocusSlug(e, "zone2_aerobic_base") && isTrueSteadyStateZone2Cardio(e)
    );
  ...
  if (top === "hills") {
    ...
    if (hillPref.length) return hillPref;
    const sprintFallback = pool.filter((e) => {
      const id = (e.id ?? "").toLowerCase();
      const name = (e.name ?? "").toLowerCase();
      const burpeeLike = id.includes("burpee") || name.includes("burpee");
      if (burpeeLike) return false;
      if (id.includes("sprint") || name.includes("sprint")) return true;
      if (id.includes("shuttle") || name.includes("shuttle")) return true;
      if (id.includes("run") || name.includes("run")) return true;
      if (id.includes("skip") || name.includes("skip")) return true;
      return false;
    });
    return sprintFallback.length ? sprintFallback : pool;
  }
  ...
  if (top === "zone2_aerobic_base" || top === "zone2_long_steady") {
    ...
    if (strict.length) return strict;
    const soft = pool.filter(
      (e) =>
        exerciseHasSubFocusSlug(e, "zone2_aerobic_base") &&
        !isSprintBurstConditioning(e) &&
        !isExplosiveConditioning(e) &&
        !/\b(quarter\s*arc|lateral\s*(high\s*|\s*)knee|agility|cone|shuffle|piston)\b/i.test(`${e.id} ${e.name}`)
    );
...
function isAgilityPlyoOrSkillDrillConditioning(exercise: Exercise): boolean {
  if (exercise.modality !== "conditioning") return false;
  const id = (exercise.id ?? "").toLowerCase();
  const name = (exercise.name ?? "").toLowerCase();
  const blob = `${id} ${name}`;
  if (
    /\b(lateral|hurdle|agility|cone|ladder|footwork|reactive|reaction|shuffle|shuttle|sprint|bound|hop|jump|skater|cutting|change[_\s-]?of[_\s-]?direction|cod|deceleration|acceleration|high[_\s-]?knee|butt[_\s-]?kick|piston)\b/.test(
      blob
    )
  ) {
    return true;
...
function isHighIntensityConditioning(exercise: Exercise): boolean {
  if (exercise.modality !== "conditioning") return false;
  if (isAgilityPlyoOrSkillDrillConditioning(exercise)) return true;
  if (HIGH_INTENSITY_CONDITIONING_IDS.has(exercise.id)) return true;
  const stimulus = (exercise.tags?.stimulus ?? []).map((s) =>
    String(s).toLowerCase().replace(/\s/g, "_")
  );
  if (stimulus.includes("plyometric")) return true;
  if (stimulus.includes("anaerobic")) {
    // Catalog Zone 2 ergs often carry anaerobic as a secondary energy tag. Do not force
    // short-interval prescriptions when the modality is steady-capable aerobic base work.
    const dualTaggedZone2 =
      stimulus.includes("aerobic_zone2") && isAllowedSteadyStateConditioning(exercise);
    if (!dualTaggedZone2) return true;
  }
...
/**
 * Steady-state modalities suitable for sustained Zone 2 labeling and continuous prescriptions.
 * Excludes agility/locomotion drills that may carry zone2 tags but are not true aerobic base work.
 * Allowed steady modalities win over noisy secondary tags (e.g. anaerobic on zone2_rower).
 */
function isTrueSteadyStateZone2Cardio(exercise: Exercise): boolean {
  if (exercise.modality !== "conditioning") return false;
  if (isExplosiveConditioning(exercise)) return false;
  if (isSprintBurstConditioning(exercise)) return false;
  if (isAllowedSteadyStateConditioning(exercise)) return true;
  if (isHighIntensityConditioning(exercise)) return false;
  return false;
}
...
function getConditioningProtocolKind(
  ...
  // Steady Zone 2 / erg cardio must never take the short HIIT protocol path.
  if (isTrueSteadyStateZone2Cardio(exercise)) {
    return primaryIntent != null ? "intent_driven" : "default_interval";
  }
...
function getConditioningStructureForExercise(
  ...
  if (nonZone2) {
    const workSeconds = structure.time_seconds ?? 0;
    const isTimed = workSeconds > 0;
    const needsShortBouts = isTimed && workSeconds > 45;
...
function buildMainHypertrophy(
  ...
  if (sportHandlesHypertrophy && snowHypertrophyRule && intentContractHype && primarySportIsAlpineSkiing(input)) {
    ...
    if (snowHypertrophyRule && snowSportAppliesH && snowKindH && chosen.length > 0) {
      const alpineHypeCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used) || chosen.some((c) => c.id === e.id));
    ...
    if (rockHypertrophyRule && rockClimbingPatternTransferApplies(input) && chosen.length > 0) {
      const rockHypeCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used) || chosen.some((c) => c.id === e.id));
      applyRockUpstreamMainLiftsCoverage(chosen, rockHypeCatalog, "main_hypertrophy");
    }
  }

  const targetPairCount = Math.floor(chosen.length / 2);
  const mainHypePairPrefs = buildSupersetIntentPreferenceScores(chosen, input);
  const rawPairs: Exercise[][] =
    targetPairCount > 0 && chosen.length >= 2
      ? (pickBestSupersetPairs(chosen, targetPairCount, new Set<string>(), undefined, mainHypePairPrefs) as [Exercise, Exercise][])
      : [];
  ...
  if (sportHandlesHypertrophy?.sportSlug === "rock_climbing" && rockHypertrophyRule && pairs.length > 0) {
    const rockHypeVolCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used) || chosen.some((c) => c.id === e.id));
    ...
    const alpineHypeVolCatalog = exercises.filter((e) => isExerciseAvailableForSession(e.id, used) || chosen.some((c) => c.id === e.id));
...
// Working volume floors live in sessionVolumeTargets (~6 working movements at 45 min).
...
function buildIntervalsHIITMain(
  ...
  const interval = getConditioningStructureByIntent(
    condMins,
    primaryIntent,
    chosen[0]?.equipment_required ?? [],
    input.primary_goal
  );

  const items: WorkoutItem[] = chosen.map((c) => {
    const p = getPrescription(
      c,
      "conditioning",
      input.energy_level,
    ...
    return {
      ...
      sets: interval.sets,
      time_seconds: interval.time_seconds ?? 45,
      rest_seconds: interval.rest_seconds,
      coaching_cues: p.coaching_cues,
      reasoning_tags: ["conditioning", "hiit", "capped_skill_drill", ...(c.tags.goal_tags ?? [])],
  ...
  const blocks: WorkoutBlock[] = [
    {
      ...
      reasoning: interval.reasoning ?? "Short capped intervals with defined rest.",
      items,
      estimated_minutes: Math.max(6, Math.round((items.length * interval.sets * ((interval.time_seconds ?? 45) + interval.rest_seconds)) / 60)),
    },
  ];
...
/** Zone 2 / long-steady intents should spend almost all non-shell time on aerobic blocks. */
function inputHasZone2SustainedIntent(input: GenerateWorkoutInput): boolean {
  const gsf = input.goal_sub_focus;
  if (!gsf) return false;
  const ranked = [
    ...(gsf.endurance ?? []),
  ...
  return ranked.some(
    (s) => s === "zone2_aerobic_base" || s === "zone2_long_steady" || s === "long_steady"
  );
}
...
function getConditioningIntentMainMinutes(input: GenerateWorkoutInput): number {
  ...
  if (isCardioPrimaryGoal(input.primary_goal)) {
    // Zone 2 / long steady: do not shrink aerobic volume for an optional joint-health pocket.
    // Accessory (when present) is additive and short; reserving 8 unused minutes left 45-min
    // sessions ~12 minutes short with only interval-length blocks.
    const accessoryReserve =
      input.duration_minutes >= 45 && !inputHasZone2SustainedIntent(input) ? 8 : 0;
...
function isHillBiasExercise(e: Exercise): boolean {
  if (e.modality !== "conditioning") return false;
  const id = (e.id ?? "").toLowerCase();
  const name = (e.name ?? "").toLowerCase();
  ...
  return uphillTreadmill || hillRun || stairMachine;
}
...
/**
 * Split Zone 2 / long-steady budgets so at least one block is a longer sustained piece
 * (~15–20 min) when the session has enough time — opener or finisher aerobic volume.
 */
function planZone2SustainedMinuteSplits(totalMinutes: number, sessionDuration: number): number[] {
  if (totalMinutes <= 0) return [];
  if (totalMinutes < 22) return [totalMinutes];

  const longPiece = Math.min(20, Math.max(15, Math.round(totalMinutes * 0.55)));
  const remainder = totalMinutes - longPiece;
  if (remainder < 8) return [totalMinutes];

  // Longer opener (aerobic progression), then a second sustained piece (easier finish volume).
  if (sessionDuration >= 45 && remainder >= 10) {
    return [longPiece, remainder];
  }
  return planConditioningBlockMinuteSplits(totalMinutes, sessionDuration);
}
...
function buildZone2SustainedMain(
  ...
  const minuteSplits = planZone2SustainedMinuteSplits(totalMainMins, input.duration_minutes ?? 45);

  const pool = buildConditioningIntentPool(exercises, {
    intentSlugs: ["zone2_aerobic_base", "zone2_long_steady"],
    used,
    overlayFilter: conditioningProfile.overlayFilter,
  });
  const pickPoolBaseRaw = pool.length
    ? pool
    : exercises.filter((e) => e.modality === "conditioning" && isExerciseAvailableForSession(e.id, used));
  // Zone 2 / long steady must use true steady cardio — never fall through to short intervals.
  const steadyPoolBase = pickPoolBaseRaw.filter((e) => isTrueSteadyStateZone2Cardio(e));
  const pickPoolBase = steadyPoolBase.length > 0 ? steadyPoolBase : pickPoolBaseRaw.filter((e) => isAllowedSteadyStateConditioning(e));
  ...
  for (let si = 0; si < minuteSplits.length; si++) {
    ...
    // Prefer variety picks; fall back to any remaining steady pool member so multi-block fill does not abort.
    const c =
      pickBestFromPool(pickPool, rng, pickCtx, ["zone2_aerobic_base", "zone2_long_steady"]) ??
      pickBestFromPool(pickPool, rng, pickCtx);
    if (!c) break;
    // Skip non-steady picks entirely for this format (do not prescribe 20–45s interval rounds).
    if (!isTrueSteadyStateZone2Cardio(c) && !isAllowedSteadyStateConditioning(c)) {
      used.add(c.id);
      continue;
    }
    used.add(c.id);
    ...
    const isLongOpener = si === 0 && mainMins >= 15 && minuteSplits.length > 1;
    const isEasyFinisher =
      minuteSplits.length > 1 && si === minuteSplits.length - 1 && !isLongOpener;
    const titleBase = isLongOpener
      ? "Zone 2 aerobic progression"
      : isEasyFinisher
        ? "Zone 2 steady finish"
    ...
    blocks.push({
      ...
      format: "straight_sets",
      title:
        si === 0 || isLongOpener || isEasyFinisher
          ? `${titleBase}${overlayLabel ? ` (${overlayLabel})` : ""}`
          : `Zone 2 sustained effort (${si + 1})${overlayLabel ? ` (${overlayLabel})` : ""}`,
      reasoning: isLongOpener
        ? "Longer sustained Zone 2 opener to fill aerobic volume for long-steady intent."
        : isEasyFinisher
          ? "Steady Zone 2 finish to accumulate aerobic minutes after the main piece."
          : "Sustained Zone 2 effort prescribed by time (no rep-based strength structure).",
      items: [
        {
          exercise_id: c.id,
          exercise_name: c.name,
  ...
  // Skip on Zone 2 / long-steady at 45 min so aerobic minutes stay near the selected duration.
  const accessoryBlocks: WorkoutBlock[] = [];
  const addAccessory =
    input.duration_minutes >= 60 ||
    (input.duration_minutes >= 45 && !inputHasZone2SustainedIntent(input));
  if (addAccessory) {
    let supportPool = exercises.filter((e) => isExerciseAvailableForSession(e.id, used));
...
function buildHillsRepeatsMain(
  ...
  const pickBase = pool.length ? pool : exercises.filter((e) => e.modality === "conditioning" && isExerciseAvailableForSession(e.id, used));
...
function buildEnduranceMain(
  ...
  const availableForPairs = strengthPool.filter((e) => isExerciseAvailableForSession(e.id, used));
  const pairPool =
    intentSlugs.length > 0
...
function isRecoveryMobilityPrimaryGoal(goal: PrimaryGoal | undefined): boolean {
  return goal === "recovery" || goal === "mobility" || goal === "recovery_mobility";
}

// --- Recovery & mobility: stretching and mobility only (no strength) ---
function buildMobilityRecoveryMain(
  ...
  const isRecoveryEmphasis =
    input.primary_goal === "recovery" ||
    input.primary_goal === "recovery_mobility";
  const mobilityPool = exercises.filter(
    (e) =>
      (e.modality === "mobility" || e.modality === "recovery") &&
      (!isRecoveryEmphasis || isRecoveryPrimaryFriendlyExercise(e)) &&
      isExerciseAvailableForSession(e.id, used)
  );
  const durationMinutes = input.duration_minutes ?? 45;
  /** Recovery & mobility primary: one unified list covers the whole session. */
  const count = isRecoveryEmphasis
  ...
  const regionalSubs =
    input.goal_sub_focus?.recovery_mobility ??
    input.goal_sub_focus?.resilience ??
    input.goal_sub_focus?.mobility ??
    [];
  const regionalGoalSlug = "recovery_mobility" as const;
    ...
    const breath =
      exercises.find(
        (e) =>
          ...
          isExerciseAvailableForSession(e.id, used) &&
          isCooldownEligibleEquipment(e.equipment_required ?? [])
      ) ?? exercises.find((e) => e.id === "breathing_cooldown" && isExerciseAvailableForSession(e.id, used));
  ...
  return [
    {
      ...
      ...(isRecoveryEmphasis
        ? {
            title: "Today's moves",
            reasoning:
              "Easy stretches, mobility, and breathing for recovery — no separate activation vs cooldown; stay gentle.",
          }
        : {}),
      items,
      estimated_minutes: isRecoveryEmphasis ? durationMinutes : Math.max(1, durationMinutes - 10),
    },
  ];
}
...
function secondaryGoalDrivingHypertrophyBlock(input: GenerateWorkoutInput): PrimaryGoal | undefined {
  for (const g of input.secondary_goals ?? []) {
    const n = g.toLowerCase().replace(/\s/g, "_");
    if (
      n.includes("hypertrophy") ||
      n.includes("body_recomp") ||
      n.includes("calisthenics") ||
      g.toLowerCase().includes("hypertrophy") ||
      g.toLowerCase().includes("calisthenics")
    ) {
      return g;
    }
  }
...
function bodyFocusShortLabel(focus: FocusBodyPart[] | undefined): string {
...
function filterTitleLeavesByBodyFocus(leaves: IntentEntry[], input: GenerateWorkoutInput): IntentEntry[] {
  if (!input.focus_body_parts?.length) return leaves;
  const subSlugs = leaves
    .filter((e) => e.kind === "goal_sub_focus" || e.kind === "sport_sub_focus")
    .map((e) => e.slug);
  const allowed = new Set(filterSubFocusSlugsForBodyFocus(subSlugs, input.focus_body_parts));
  const filtered = leaves.filter((e) => {
    if (e.kind !== "goal_sub_focus" && e.kind !== "sport_sub_focus") return true;
    return allowed.has(e.slug);
  });
  return filtered.length > 0 ? filtered : leaves;
}

/** Session title leaves: dedicated/emphasis days show primary goal subs only (not all weekly goals). */
function filterTitleLeavesForSession(input: GenerateWorkoutInput, titleLeaves: IntentEntry[]): IntentEntry[] {
  const primary = input.primary_goal;
  const primaryLeaves = titleLeaves.filter(
    (entry) =>
      (entry.kind === "goal_sub_focus" && entry.parent_slug === primary) ||
      (entry.kind === "goal" && entry.slug === primary)
  );
  const primaryWeight = primaryLeaves.reduce((sum, entry) => sum + entry.weight, 0);
  if (primaryWeight >= 0.45 && primaryLeaves.length > 0) {
    return filterTitleLeavesByBodyFocus(primaryLeaves, input);
  }
  if (isImpliedSportOnlySession(input)) {
    return titleLeaves.filter((entry) => !(entry.kind === "goal" && entry.slug === "strength"));
  }
  return titleLeaves;
}

function getRankedGoalSubFocusSlugsForGoal(input: GenerateWorkoutInput, goal: PrimaryGoal): string[] {
  const gsf = input.goal_sub_focus ?? {};
  const gw = input.goal_sub_focus_weights ?? {};
  const ranked: { slug: string; w: number }[] = [];
  for (const key of goalSubFocusKeysForPrimary(goal)) {
    const slugs = gsf[key];
    if (!slugs?.length) continue;
    const weights = gw[key] ?? slugs.map(() => 1 / slugs.length);
    for (let i = 0; i < slugs.length; i++) {
      ranked.push({ slug: slugs[i]!, w: weights[i] ?? 0 });
    }
  }
  ...
  return filterSubFocusSlugsForBodyFocus(out, input.focus_body_parts);
}
...
function trimBlocksToDurationBudget(
  ...
  const trimmed = blocks.map((b) => ({ ...b, items: [...b.items] }));
  const sumMinutes = () => trimmed.reduce((sum, b) => sum + (b.estimated_minutes ?? 5), 0);
  let running = sumMinutes();
  ...
  if (options?.preserveCooldown) {
    const hasCooldown = trimmed.some((b) => b.block_type === "cooldown");
    if (!hasCooldown) {
      const originalCooldown = blocks.find((b) => b.block_type === "cooldown");
      if (originalCooldown) trimmed.push({ ...originalCooldown, items: [...originalCooldown.items] });
    }
  }
  return trimmed;
}
...
function sessionHasGoalSubFocusCoverage(
  ...
  for (const id of ids) {
    ...
    for (const slug of guaranteeSlugs) {
      if (exerciseMatchesSelectedSubGoalStrict(ex, goalSlug, slug)) return true;
    }
  }
  return false;
}

function exerciseMatchesSelectedSubGoalStrict(exercise: Exercise, goalSlug: string, subGoalSlug: string): boolean {
  const normSubGoal = subGoalSlug.toLowerCase().replace(/\s+/g, "_");
  const id = (exercise.id ?? "").toLowerCase();
  const name = (exercise.name ?? "").toLowerCase();
  if (goalSlug === "strength") {
    // For canonical strength intents, require specific intent-pattern evidence.
    if (normSubGoal === "overhead_press") {
      return (
    ...
    if (normSubGoal === "squat") {
      return name.includes("squat") || id.includes("squat");
    }
    if (normSubGoal === "deadlift_hinge") {
      return (
    ...
    return exerciseHasStrengthSubFocusSlug(exercise, subGoalSlug);
  }
  if (goalSlug === "conditioning" || goalSlug === "endurance") {
    return exerciseHasSubFocusSlug(exercise, subGoalSlug);
  }
  return exerciseMatchesGoalSubFocusSlugUnified(exercise, goalSlug, subGoalSlug);
}

function exerciseMatchesSelectedSubGoalWithFallback(
  exercise: Exercise,
  goalSlug: string,
  subGoalSlug: string
): boolean {
  if (exerciseMatchesSelectedSubGoalStrict(exercise, goalSlug, subGoalSlug)) return true;
  const id = (exercise.id ?? "").toLowerCase();
  const name = (exercise.name ?? "").toLowerCase();
  const normSubGoal = subGoalSlug.toLowerCase().replace(/\s+/g, "_");
  if (goalSlug === "strength") {
...
function ensureSelectedSportSubFocusCoverage(
  ...
  const coveragePrescriptionContext = (subSlug: string): { blockType: BlockType; primaryGoal: PrimaryGoal } => {
    ...
    if (isExplosivePlyometricSportSubFocusSlug(canon)) return { blockType: "power", primaryGoal: "power" };
    if (canon === "speed" || canon === "change_of_direction" || canon === "reactive_speed" || canon === "speed_power") {
      return { blockType: "power", primaryGoal: "power" };
    }
    if (isStabilityPrehabSportSubFocusSlug(canon)) return { blockType: "accessory", primaryGoal: "recovery" };
  ...
  const scoreCoverageCandidate = (exercise: Exercise, subSlug: string): number => {
    ...
    let score = subFocusExerciseSelectionScore(exercise, subSlug);
  ...
  const subFocusPairCoverageCount = (sportKey: string, subSlug: string): number => {
    ...
    const requiresHighQualityCoverage =
      isExplosivePlyometricSportSubFocusSlug(canonSub) ||
      isStabilityPrehabSportSubFocusSlug(canonSub) ||
      canonSub === "speed" ||
      canonSub === "change_of_direction" ||
      canonSub === "reactive_speed" ||
      canonSub === "speed_power";
    let n = 0;
    for (const id of collectExerciseIdsForSubFocusCoverage(mergedBlocks)) {
      const ex = exerciseById.get(id);
      if (!ex || !exerciseMatchesSportSubFocusForCoverage(ex, sportKey, subSlug)) continue;
      if (requiresHighQualityCoverage && scoreCoverageCandidate(ex, subSlug) < 10) {
        continue;
      }
      n++;
    }
  ...
  for (const sportKey of sportKeysForSubPairs) {
    ...
    for (const subSlug of slugs) {
      ...
      const matchingPool = sportIntentCandidatePool.filter(
        (e) =>
          isExerciseAvailableForSession(e.id, used) &&
          exerciseMatchesSportSubFocusForCoverage(e, sportKey, subSlug) &&
          (!fillConstraints ||
            isExerciseEligibleForBlock(e, {
              blockType: coveragePrescriptionContext(subSlug).blockType,
              constraints: fillConstraints,
              input,
            }))
      );
  ...
  const hasPowerCoverage = itemsToAdd.some((item) =>
    item.reasoning_tags?.some((t) => {
      const c = normalizeSubFocusSlug(String(t));
      return (
        c === "speed" ||
        c === "change_of_direction" ||
        isExplosivePlyometricSportSubFocusSlug(c)
      );
    })
  );
  const coverageBlockType: BlockType = hasPowerCoverage ? "power" : "accessory";

  const cooldownIdx = mergedBlocks.findIndex((b) => b.block_type === "cooldown");
  const newBlock: WorkoutBlock = {
    block_type: coverageBlockType,
    format: "straight_sets",
...
/**
 * Proportional sport-intent repair.
 *
 * Product contract: if the loading screen / title says "Climbing 50%", a visible share
 * of working exercises must actually be climbing-tagged. Earlier coverage only ensured
 * "at least one sport exercise"; this pass replaces generic working items with
 * sport-matched alternatives until the selected sport share is represented.
 */
function enforceSportIntentExerciseProportions(
  mergedBlocks: WorkoutBlock[],
  input: GenerateWorkoutInput,
  sportIntentCandidatePool: Exercise[],
  guaranteePool: Exercise[],
  ...
  const sportWeight = Math.max(0, Math.min(1, input.sport_weight ?? input.session_intent?.sport_weight ?? 0));
  if (sportWeight <= 0) return;

  const rankedSportsRaw =
    input.session_intent?.selected_sports && input.session_intent.selected_sports.length > 0
      ? input.session_intent.selected_sports
      : input.sport_slugs ?? [];
  const rankedSports = [...new Set(rankedSportsRaw.map((s) => getCanonicalSportSlug(s)))];
  if (rankedSports.length === 0) return;
  ...
  const blockReplacePriority = (blockType: BlockType): number => {
    if (blockType === "accessory") return 0;
    if (blockType === "conditioning") return 1;
    if (blockType === "main_hypertrophy") return 2;
    if (blockType === "main_strength") return 3;
    return 4;
  };

  const makeItem = (ex: Exercise, blockType: BlockType, sportSlug: string): WorkoutItem => {
    const p = getPrescription(
      ex,
      blockType,
      input.energy_level,
    ...
    return {
      ...
      rest_seconds: p.rest_seconds,
      coaching_cues: p.coaching_cues,
      reasoning_tags: ["sport_intent_proportion", sportSlug, ...(ex.tags.goal_tags ?? [])],
      unilateral: ex.unilateral ?? false,
    };
  };

  const resolvePrimarySportForSlot = (slot: Slot): string | undefined => {
    const ex = slot.ex;
    if (!ex) return undefined;
    for (const sport of rankedSports) {
      if (exerciseMatchesUserSportTagExercise(ex, sport)) return sport;
    }
    return undefined;
  };

  const replaceSlotExercise = (slot: Slot, next: Exercise, sportSlug: string): void => {
    used.delete(slot.item.exercise_id);
    used.add(next.id);
    const nextItem = makeItem(next, slot.block.block_type as BlockType, sportSlug);
    const previousId = slot.item.exercise_id;
    slot.block.items[slot.itemIndex] = nextItem;
    if (slot.block.supersetPairs?.length) {
      slot.block.supersetPairs = slot.block.supersetPairs.map((pair) =>
        pair.map((item) => (item.exercise_id === previousId ? nextItem : item)) as [WorkoutItem, WorkoutItem]
      );
    }
  ...
  const totalSportSlots = Math.max(1, Math.round(slots.length * sportWeight));
  const perSportExact = totalSportSlots / rankedSports.length;
  const targetBySport: Record<string, number> = {};
  let allocated = 0;
  const remainders: Array<{ sport: string; remainder: number }> = [];
  for (const sport of rankedSports) {
    const base = Math.floor(perSportExact);
    targetBySport[sport] = base;
    allocated += base;
    remainders.push({ sport, remainder: perSportExact - base });
  }
  remainders.sort((a, b) => b.remainder - a.remainder);
  let remaining = totalSportSlots - allocated;
  let r = 0;
  while (remaining > 0 && remainders.length > 0) {
    const sport = remainders[r % remainders.length]!.sport;
    targetBySport[sport] = (targetBySport[sport] ?? 0) + 1;
    remaining--;
    r++;
  }
  ...
  const countBySport = (): Record<string, number> => {
    ...
    for (const sport of rankedSports) counts[sport] = 0;
    for (const slot of workingSlots()) {
      const sport = resolvePrimarySportForSlot(slot);
      if (sport) counts[sport] = (counts[sport] ?? 0) + 1;
    }
    return counts;
  };

  const pickReplacementCandidate = (sport: string, blockType: BlockType): Exercise | undefined => {
    let candidates = sportIntentCandidatePool.filter(
      (e) =>
        isExerciseAvailableForSession(e.id, used) &&
    ...
    if (activeSubs.length > 0) {
      candidates = [...candidates].sort(
        (a, b) =>
          aggregateSubFocusSelectionScore(b, activeSubs) - aggregateSubFocusSelectionScore(a, activeSubs)
      );
      const topScore = aggregateSubFocusSelectionScore(candidates[0]!, activeSubs);
      const tier = candidates.filter(
        (c) => aggregateSubFocusSelectionScore(c, activeSubs) >= topScore - 1
      );
  ...
  const chooseSlotToReplace = (
    desiredSport: string,
    counts: Record<string, number>
  ): Slot | undefined => {
    const all = workingSlots().map((slot) => ({
      slot,
      sport: resolvePrimarySportForSlot(slot),
    }));
    const underTargetSports = new Set(
      rankedSports.filter((s) => (counts[s] ?? 0) < (targetBySport[s] ?? 0))
    );
    // Prefer truly generic slots first so we do not steal from another under-target sport.
    const generic = all
      .filter((row) => row.sport == null)
      .map((row) => row.slot)
      .sort((a, b) => {
        const p = blockReplacePriority(a.block.block_type as BlockType) - blockReplacePriority(b.block.block_type as BlockType);
        if (p !== 0) return p;
        return a.blockIndex - b.blockIndex || a.itemIndex - b.itemIndex;
      });
    if (generic.length > 0) return generic[0];

    // Then allow replacing from over-target sports only.
    const overTarget = all
      .filter((row) => {
        if (!row.sport) return false;
        if (row.sport === desiredSport) return false;
        if (underTargetSports.has(row.sport)) return false;
        return (counts[row.sport] ?? 0) > (targetBySport[row.sport] ?? 0);
      })
      .map((row) => row.slot)
      .sort((a, b) => {
        const p = blockReplacePriority(a.block.block_type as BlockType) - blockReplacePriority(b.block.block_type as BlockType);
    ...
    if (overTarget.length > 0) return overTarget[0];
    return undefined;
  };

  // Iteratively fill deficits until stable or no feasible moves remain.
  for (let guard = 0; guard < slots.length * 3; guard++) {
    const counts = countBySport();
    const deficits = rankedSports
      .map((sport) => ({
        sport,
        need: (targetBySport[sport] ?? 0) - (counts[sport] ?? 0),
      }))
      .filter((d) => d.need > 0)
    ...
    for (const d of deficits) {
      const slot = chooseSlotToReplace(d.sport, counts);
      if (!slot) continue;
      const candidate = pickReplacementCandidate(d.sport, slot.block.block_type as BlockType);
      if (!candidate) continue;
      replaceSlotExercise(slot, candidate, d.sport);
      progressed = true;
      break;
    }
    if (!progressed) break;
    ...
    for (const v of ev.violations) {
      ...
      if (v.ruleId.endsWith("conditioning_relevance")) {
        ...
        let altPool = filtered.filter(
          (e) => e.modality === "conditioning" && isSnowSportConditioningExercise(e, kind) && isExerciseAvailableForSession(e.id, sid)
        );
        if (altPool.length === 0) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    for (const v of ev.violations) {
      ...
      if (v.ruleId === "hiking_conditioning_relevance") {
        ...
        sid.delete(item.exercise_id);
        let altPool = filtered.filter(
          (e) => e.modality === "conditioning" && isHikingConditioningExercise(e) && isExerciseAvailableForSession(e.id, sid)
        );
        if (altPool.length === 0) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    for (const v of ev.violations) {
      ...
      if (v.ruleId === "trail_conditioning_relevance") {
        ...
        sid.delete(item.exercise_id);
        let altPool = filtered.filter(
          (e) => e.modality === "conditioning" && isTrailRunningConditioningExercise(e) && isExerciseAvailableForSession(e.id, sid)
        );
        if (altPool.length === 0) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    for (const v of ev.violations) {
      ...
      if (v.ruleId === "road_conditioning_relevance") {
        ...
        sid.delete(item.exercise_id);
        let altPool = filtered.filter(
          (e) => e.modality === "conditioning" && isRoadRunningConditioningExercise(e) && isExerciseAvailableForSession(e.id, sid)
        );
        if (altPool.length === 0) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    for (const v of ev.violations) {
      ...
      if (v.ruleId === "soccer_conditioning_rsa_relevant") {
        ...
        sid.delete(item.exercise_id);
        let altPool = filtered.filter(
          (e) => e.modality === "conditioning" && isSoccerConditioningExercise(e) && isExerciseAvailableForSession(e.id, sid)
        );
        if (altPool.length === 0) {
...
function sessionTitle(input: GenerateWorkoutInput): string {
  ...
  // Use declared intent leaf entries; dedicated days show primary goal + body-relevant subs only.
  const rankedEntries = input.session_intent?.ranked_intent_entries ?? [];
  const titleLeaves = deriveLeafEntries(rankedEntries);
  if (titleLeaves.length > 0) {
    const leavesForTitle = filterTitleLeavesForSession(input, titleLeaves);
    const labels = leavesForTitle.map((entry) => {
    ...
    const focusLabel = bodyFocusShortLabel(input.focus_body_parts);
    const parts = focusLabel ? [...unique, focusLabel] : unique;
    return `${parts.filter(Boolean).join(" · ")} • ${dur}`;
  }
...
function ensureBlockGoalIntentFallbacks(
  blocks: WorkoutBlock[],
  input: GenerateWorkoutInput,
  exerciseById: Map<string, Exercise>
): void {
  const selectedGoals =
    input.session_intent?.selected_goals?.length
      ? input.session_intent.selected_goals
  ...
  for (const block of blocks) {
    ...
    const matchingSwapIds = Array.from(
      new Set(
        [...exerciseById.values()]
          .filter((ex) => exerciseMatchesDeclaredGoal(ex, goalSlug))
          .map((ex) => ex.id)
      )
    );
    const itemIds = block.items.map((item) => item.exercise_id).filter((id) => exerciseById.has(id));
    block.goal_intent = {
      intent_kind: "goal",
      goal_slug: goalSlug,
      swap_pool_exercise_ids: capSwapPoolIds(
        matchingSwapIds.length > 0 ? matchingSwapIds : itemIds,
        GOAL_DEDICATED_SWAP_POOL_CAP
      ),
    };
  }
...
export function generateWorkoutSession(
  ...
  if (primary === "strength" && (input.goal_sub_focus?.[primary]?.length ?? 0) > 0) {
    ...
    const rankedCal =
      input.goal_sub_focus?.calisthenics?.length ? input.goal_sub_focus.calisthenics : input.goal_sub_focus?.strength;
    if (rankedCal?.length) {
      strengthProfileForWarmup = resolveSubFocusProfile({
        goalSlug: "calisthenics",
        rankedSubFocusSlugs: rankedCal,
        rankWeights:
          input.goal_sub_focus_weights?.calisthenics ?? input.goal_sub_focus_weights?.strength,
      });
    }
  }
  ...
  const blendedManualStrength = tryBuildBlendedManualStrengthReserve(
    filtered,
    input,
    used,
    recentIds,
  ...
  if (blendedManualStrength?.length) {
    blocks.push(...blendedManualStrength);
  }
  ...
  const intentLeavesForUnified = deriveLeafEntries(input.session_intent?.ranked_intent_entries ?? []);
  const useUnifiedIntentSlots =
    (!hasSportPatternTransferForGoalDedicated(input) ||
      sportTransferYieldToSpeedPowerIntentSlots(input)) &&
    (intentLeavesForUnified.length >= 2 ||
      (intentLeavesForUnified.length === 1 &&
        isPowerStyleSportIntentEntry(intentLeavesForUnified[0]!)));
  const unifiedSpecialtyResult = useUnifiedIntentSlots
  ...
  if (sessionAssemblesPowerBlock(input)) {
    ...
    blocks.push(
      ...buildMainStrength(
        ...
        mainSelectorTrace,
        blockIntentProfile.targetCardioExerciseShare,
        strengthGoalDedicatedInject ?? undefined
      )
    );
  ...
  } else if (isRecoveryMobilityPrimaryGoal(primary)) {
    // Recovery & mobility stretches should follow regional sub-focus (e.g. hips) even when body-part
  ...
  // 5c. Power block when power is secondary goal or speed/sprint intent requires it
  if (constraints.prefer_power_block && primary !== "power") {
    const hasPowerBlock = blocks.some((b) => b.block_type === "power");
    if (!hasPowerBlock) {
      blocks.push(
        ...buildPowerBlock(
          filtered,
  ...
  const preferEnduranceIntentForFinisher =
    requiredConditioning && primary !== "endurance" && secondaryGoals.includes("endurance");
  const rankedCardioIntentsFinisher = getCardioFinisherIntentSlugs(
    input,
    ...
    if (addConditioning && conditioningMins > 0) {
      ...
      if (cardioPool.length) {
        const narrowedPool = rankedCardioIntentsFinisher?.length
          ...
          : cardioPool;
        let c = pickConditioningExercise(
          narrowedPool,
          input.style_prefs?.preferred_zone2_cardio,
          rng,
        ...
        if (
          ...
          rankedCardioIntentsFinisher?.includes("hills") &&
          !isHillBiasExercise(c)
        ) {
          const hillCandidatePool = narrowedPool.filter((e) => isHillBiasExercise(e));
          const fallbackHillPool =
            hillCandidatePool.length > 0
              ? hillCandidatePool
              : guaranteePool.filter(
                  (e) =>
                    isExerciseAvailableForSession(e.id, used) &&
                    isHillBiasExercise(e) &&
                    isConditioningBlockPoolCandidate(e, { input })
                );
          if (fallbackHillPool.length > 0) {
            c = fallbackHillPool[Math.floor(rng() * fallbackHillPool.length)] ?? c;
          }
        }
        if (c) {
          used.add(c.id);
          ...
          const finisherCondIntent = rankedCardioIntentsFinisher?.[0];
          const interval = getConditioningStructureForExercise(
            c,
            conditioningMins,
            input.primary_goal,
          ...
          const conditioningTag = conditioningProtocolReasonTag(
            getConditioningProtocolKind(c, finisherCondIntent)
          );
          const condFormat =
            (interval.format as BlockFormat) ??
          ...
          const conditioningBlockTitle =
            isTrueSteadyStateZone2Cardio(c)
              ? "Zone 2"
  ...
  if (!isRecoveryMobilityPrimaryGoal(primary) && primary !== "joint_health") {
    const cooldown = buildCooldown(filtered, input, used, rng, {
      ...
      fallbackPool: guaranteePool,
      requiresCooldown: blockIntentProfile.requiresCooldownBlock,
      minCooldownItems: blockIntentProfile.minCooldownItems,
    });
    if (cooldown.items.length > 0 || blockIntentProfile.requiresCooldownBlock) {
  ...
  if (
    ...
    !hypertrophyPrimaryExcludesConditioning(input) &&
    !mergedBlocks.some((b) => b.block_type === "conditioning")
  ) {
    const existingIds = new Set(mergedBlocks.flatMap((b) => b.items.map((i) => i.exercise_id)));
    const uniqueCandidates = new Map<string, Exercise>();
    const conditioningCtx = { input };
    for (const src of [filtered, hardFiltered, effectiveExercisePool, exercisePool]) {
      for (const e of src) {
        if (!isConditioningBlockPoolCandidate(e, conditioningCtx)) continue;
        if (existingIds.has(e.id)) continue;
        if (uniqueCandidates.has(e.id)) continue;
        const shape = toConstraintEligibilityShape(e);
        if (!isExerciseAllowedByInjuries(shape, constraints)) continue;
        uniqueCandidates.set(e.id, e);
      }
    }
    const conditioningPool = [...uniqueCandidates.values()].filter((e) => isExerciseAvailableForSession(e.id, used));
    let c = pickConditioningExercise(
      conditioningPool,
      input.style_prefs?.preferred_zone2_cardio,
      rng,
    ...
    if (!c) {
      c = [...uniqueCandidates.values()].find(
        (e) => e.modality !== "recovery" && isConditioningBlockPoolCandidate(e, conditioningCtx)
      );
    }
    if (c) {
      used.add(c.id);
      ...
      const condIntent = rankedCardioIntentsFinisher?.[0];
      const interval = getConditioningStructureForExercise(
        c,
        Math.max(6, Math.min(14, Math.round((input.duration_minutes ?? 45) * 0.25))),
        input.primary_goal,
        condIntent
      );
      mergedBlocks.push({
        block_type: "conditioning",
        format: (interval.format as BlockFormat) ?? "straight_sets",
        title: isTrueSteadyStateZone2Cardio(c) ? "Zone 2" : "Conditioning",
        reasoning: "Required conditioning block preserved after duration trims.",
        items: [
          {
            exercise_id: c.id,
            exercise_name: c.name,
            ...
            rest_seconds: interval.rest_seconds,
            coaching_cues: p.coaching_cues,
            reasoning_tags: ["conditioning", "required_conditioning_block", ...(c.tags.goal_tags ?? [])],
            unilateral: c.unilateral ?? false,
          },
        ],
        estimated_minutes: Math.max(4, Math.round((input.duration_minutes ?? 45) * 0.2)),
      });
    }
  }
  const cooldownRequired =
    blockIntentProfile.requiresCooldownBlock || (constraints.min_cooldown_mobility_exercises ?? 0) > 0;
  const minCooldownTarget = Math.max(
    blockIntentProfile.minCooldownItems,
  ...
  if (cooldownRequired && !isRecoveryMobilityPrimaryGoal(primary) && primary !== "joint_health") {
    ...
    if (!existingCooldown || existingCount < minCooldownTarget) {
      const rebuiltCooldown = buildCooldown(filtered, input, used, rng, {
        constraints,
        mainWorkFamilies: getMainWorkFamiliesFromBlocks(mergedBlocks, filtered),
        preferredTargets: blockIntentProfile.cooldownPreferredTargets,
        fallbackPool: guaranteePool,
        requiresCooldown: true,
        minCooldownItems: minCooldownTarget,
      });
      if (existingCooldown) {
        const existingIds = new Set(existingCooldown.items.map((i) => i.exercise_id));
        for (const item of rebuiltCooldown.items) {
          if (existingIds.has(item.exercise_id)) continue;
          existingCooldown.items.push(item);
          existingIds.add(item.exercise_id);
        }
        existingCooldown.estimated_minutes = Math.max(
          existingCooldown.estimated_minutes ?? 0,
          rebuiltCooldown.estimated_minutes ?? 0
        );
      } else if (rebuiltCooldown.items.length > 0) {
        mergedBlocks.push(rebuiltCooldown);
      }
    }
    mergedBlocks = removeEmptyBlocks(mergedBlocks);
    if (!mergedBlocks.some((b) => b.block_type === "cooldown")) {
      const minMobilityNeeded = Math.max(1, minCooldownTarget);
      const primaryCandidates = buildCooldownStretchPool(
        filtered.filter((e) => isCooldownEligibleEquipment(e.equipment_required ?? [])),
        { alreadyUsedIds: used }
      );
      const fallbackCandidates =
        primaryCandidates.length >= minMobilityNeeded
          ? primaryCandidates
          : buildCooldownStretchPool(
              guaranteePool.filter((e) => isCooldownEligibleEquipment(e.equipment_required ?? [])),
              { alreadyUsedIds: used }
            );
      const selectedFallback = selectCooldownExercisesWithFallback(
        primaryCandidates,
        fallbackCandidates,
        {
          minMobilityCount: minMobilityNeeded,
          preferredTargets: [
            ...blockIntentProfile.cooldownPreferredTargets,
            ...getGeneralCooldownFallbackTargets(input.focus_body_parts ?? []),
          ],
          alreadyUsedIds: used,
          rng,
          maxItems: minMobilityNeeded,
        }
      );
      if (selectedFallback.length > 0) {
        const items: WorkoutItem[] = selectedFallback.slice(0, minMobilityNeeded).map((exercise) => {
          ...
          return {
            ...
            rest_seconds: 0,
            coaching_cues: p.coaching_cues,
            reasoning_tags: ["cooldown", "recovery", ...(exercise.tags.goal_tags ?? [])],
        ...
        mergedBlocks.push({
          ...
          reasoning: "Fallback cooldown to preserve recovery/mobility requirement.",
          items,
          estimated_minutes: Math.max(2, selectedFallback.length * 2),
        });
      }
    }
  }

  // 7b) Post-assembly quality guards: ensure minimum training density and secondary-goal representation.
  const filteredById = new Map(filtered.map((e) => [e.id, e]));
  const fullEligibleById = new Map(
    [...filtered, ...guaranteePool, ...sportIntentRescuePool].map((e) => [e.id, e])
  );
  const nonSupportBlock = (bt: string) =>
    bt !== "warmup" && bt !== "cooldown" && bt !== "mobility" && bt !== "recovery";
  const minWorkItemsByDuration =
    input.duration_minutes <= 30 ? 3 : input.duration_minutes <= 45 ? 5 : input.duration_minutes <= 60 ? 6 : 7;

  const countWorkItems = () =>
    mergedBlocks.reduce(
      (n, b) => n + (nonSupportBlock(b.block_type) ? b.items.length : 0),
      0
    );
  ...
  const findUnusedGoalMatch = (goal: PrimaryGoal): Exercise | null => {
    for (const ex of filtered) {
      if (!isExerciseAvailableForSession(ex.id, used)) continue;
      const shape = toConstraintEligibilityShape(ex);
      if (!isExerciseAllowedByInjuries(shape, constraints)) continue;
      if ((goal === "conditioning" || goal === "endurance") && !isConditioningEligible(ex)) continue;
      if (isRecoveryMobilityPrimaryGoal(goal) && !isRecoveryCooldownEligible(ex)) continue;
      if (goal === "joint_health") {
        if (isJointHealthExcludedExercise(ex) || !isJointHealthAppropriateExercise(ex)) continue;
      }
      if (isRecoveryMobilityPrimaryGoal(goal)) {
        if (isRecoveryMobilityPrimaryGoal(input.primary_goal)) {
  ...
  const ensureAccessoryBoosterBlock = (): WorkoutBlock => {
    let block = mergedBlocks.find((b) => b.block_type === "accessory");
    if (!block) {
      block = {
        block_type: "accessory",
        format: "straight_sets",
        title: "Accessory (coverage)",
        reasoning: "Post-assembly coverage booster for structure and goal balance.",
        items: [],
        estimated_minutes: 0,
      };
  ...
  for (const sg of input.secondary_goals ?? []) {
    ...
    const targetBlockType: BlockType =
      sg === "conditioning" || sg === "endurance"
        ? "conditioning"
        : isRecoveryMobilityPrimaryGoal(sg)
          ? "cooldown"
        : sg === "joint_health"
          ? "accessory"
        : sg === "hypertrophy" || sg === "body_recomp" || sg === "calisthenics"
          ? "main_hypertrophy"
          : "accessory";
    const p = getPrescription(
      candidate,
      targetBlockType === "main_hypertrophy" ? "main_hypertrophy" : targetBlockType,
      input.energy_level,
      sg,
      false,
      fatigueVolumeScale,
    ...
    const newItem: WorkoutItem = {
      ...
      rest_seconds: p.rest_seconds,
      coaching_cues: p.coaching_cues,
      reasoning_tags: ["secondary_goal_coverage", sg, ...(candidate.tags.goal_tags ?? [])],
      unilateral: candidate.unilateral ?? false,
    };
    used.add(candidate.id);
    if (targetBlockType === "conditioning") {
      const cond = mergedBlocks.find((b) => b.block_type === "conditioning");
      if (cond) {
        cond.items.push(newItem);
        cond.estimated_minutes = (cond.estimated_minutes ?? 0) + 6;
      } else {
        mergedBlocks.push({
          block_type: "conditioning",
          format: "straight_sets",
    ...
    } else if (targetBlockType === "cooldown") {
      const cool = mergedBlocks.find((b) => b.block_type === "cooldown");
      if (cool) {
        cool.items.push(newItem);
        cool.estimated_minutes = (cool.estimated_minutes ?? 0) + 3;
      } else {
        mergedBlocks.push({
          block_type: "cooldown",
          format: "circuit",
    ...
    } else if (targetBlockType === "main_hypertrophy") {
      const hyp = mergedBlocks.find((b) => b.block_type === "main_hypertrophy");
      if (hyp) {
        hyp.items.push(newItem);
        hyp.estimated_minutes = (hyp.estimated_minutes ?? 0) + 7;
      } else {
        mergedBlocks.push({
          block_type: "main_hypertrophy",
          format: "straight_sets",
  ...
  const skipDensityBooster =
    isRecoveryMobilityPrimaryGoal(primary) ||
    primary === "joint_health" ||
    blockIntentProfile.suppressAccessoryBlocks;

  if (blockIntentProfile.suppressAccessoryBlocks) {
    mergedBlocks = mergedBlocks.filter((b) => b.block_type !== "accessory");
  }

  if (
    blockIntentProfile.requiresAccessoryBlocks &&
    !blockIntentProfile.suppressAccessoryBlocks &&
    !mergedBlocks.some((b) => b.block_type === "accessory")
  ) {
    const accessoryCandidates = filtered.filter(
      (e) =>
        isAccessoryEligible(e) &&
        isExerciseAvailableForSession(e.id, used) &&
        !(e.exercise_role && MAIN_WORK_EXCLUDED_ROLES.has(e.exercise_role.toLowerCase().replace(/\s/g, "_")))
    );
    const pool = accessoryCandidates.filter((ex) => {
      const shape = toConstraintEligibilityShape(ex);
      return isExerciseAllowedByInjuries(shape, constraints);
    });
    if (pool.length >= 2) {
      const accessoryPairPrefs = buildSupersetIntentPreferenceScores(pool, input, {
        blockType: "accessory",
      });
      const pairs = pickBestSupersetPairs(pool, 1, used, rng, accessoryPairPrefs) as [Exercise, Exercise][];
      if (pairs.length > 0) {
        const [exA, exB] = pairs[0]!;
        used.add(exA.id);
        used.add(exB.id);
        ...
        const itemA: WorkoutItem = {
          ...
          rest_seconds: pA.rest_seconds,
          coaching_cues: pA.coaching_cues,
          reasoning_tags: ["accessory", "hypertrophy_support", ...(exA.tags.goal_tags ?? [])],
          unilateral: exA.unilateral ?? false,
        };
        const itemB: WorkoutItem = {
          exercise_id: exB.id,
          ...
          rest_seconds: pB.rest_seconds,
          coaching_cues: pB.coaching_cues,
          reasoning_tags: ["accessory", "hypertrophy_support", ...(exB.tags.goal_tags ?? [])],
          unilateral: exB.unilateral ?? false,
        };
        mergedBlocks.push({
          block_type: "accessory",
          format: "superset",
          title: "Accessory",
          reasoning: "Dedicated accessory volume for hypertrophy-focused sessions.",
          items: [itemA, itemB],
          supersetPairs: [[itemA, itemB]],
          estimated_minutes: 8,
        });
        ...
        items: [
          {
            ...
            rest_seconds: p.rest_seconds,
            coaching_cues: p.coaching_cues,
            reasoning_tags: ["accessory", "hypertrophy_support", ...(ex.tags.goal_tags ?? [])],
            unilateral: ex.unilateral ?? false,
          },
        ],
        estimated_minutes: 6,
  ...
  while (!skipDensityBooster && countWorkItems() < minWorkItemsByDuration) {
    const candidate = filtered.find((ex) => {
      if (!isExerciseAvailableForSession(ex.id, used)) return false;
      const shape = toConstraintEligibilityShape(ex);
      if (!isExerciseAllowedByInjuries(shape, constraints)) return false;
      if (
        constraints.allowed_movement_families != null &&
        constraints.allowed_movement_families.length > 0 &&
        !matchesBodyPartFocus(shape, constraints, "accessory")
      ) {
        return false;
      }
      return ex.modality !== "recovery";
    });
    if (!candidate) break;
    used.add(candidate.id);
    const p = getPrescription(
    ...
    acc.items.push({
      ...
      rest_seconds: p.rest_seconds,
      coaching_cues: p.coaching_cues,
      reasoning_tags: ["density_booster", ...(candidate.tags.goal_tags ?? [])],
      unilateral: candidate.unilateral ?? false,
    });
    acc.estimated_minutes = (acc.estimated_minutes ?? 0) + 5;
  }
  mergedBlocks = removeEmptyBlocks(mergedBlocks);

  // Final sport-coverage repair pass after trimming/required-block insertions.
  tryRepairHikingSession(
    mergedBlocks,
    input,
    filtered,
  ...
  enforceSportIntentExerciseProportions(
    mergedBlocks,
    input,
    sportIntentRescuePool,
    guaranteePool,
  ...
  // Full-body is a coverage contract, not merely permission to select from every region.
  // Scoring can otherwise fill every working slot from one high-scoring region. Preserve the
  // user's explicit Full selection by guaranteeing distinct upper, lower, and core work.
  const normalizedBodyFocus = (input.focus_body_parts ?? []).map((part) =>
    String(part).toLowerCase().trim().replace(/\s+/g, "_")
  );
  const fullBodyCoverageGoals = new Set<PrimaryGoal>([
    "strength",
    "hypertrophy",
    "body_recomp",
    "calisthenics",
  ...
  if (
    normalizedBodyFocus.includes("full_body") &&
    fullBodyCoverageGoals.has(input.primary_goal)
  ) {
    type FullBodyRegion = "upper" | "lower" | "core";
    const exerciseRegions = (exercise: Exercise): Set<FullBodyRegion> => {
      const families = getEffectiveMovementFamilies(toConstraintEligibilityShape(exercise));
      const regions = new Set<FullBodyRegion>();
    ...
    const covered = new Set<FullBodyRegion>();
    for (const block of mergedBlocks) {
      if (!nonSupportBlock(block.block_type)) continue;
      for (const item of block.items) {
        const exercise = fullEligibleById.get(item.exercise_id);
        if (!exercise) continue;
        for (const region of exerciseRegions(exercise)) covered.add(region);
      }
    }

    const missing = (["upper", "lower", "core"] as const).filter(
      (region) => !covered.has(region)
    );
    if (missing.length > 0) {
      const coverageItems: WorkoutItem[] = [];
      const coveragePool = [...fullEligibleById.values()];
      for (const region of missing) {
        const candidate = coveragePool.find((exercise) => {
          if (!isExerciseAvailableForSession(exercise.id, used)) return false;
          if (!exerciseRegions(exercise).has(region)) return false;
          const role = exercise.exercise_role?.toLowerCase().replace(/\s/g, "_");
          if (role && MAIN_WORK_EXCLUDED_ROLES.has(role)) return false;
          return exercise.modality !== "recovery" && exercise.modality !== "mobility";
        });
        if (!candidate) continue;
        used.add(candidate.id);
        ...
        coverageItems.push({
          ...
          coaching_cues: prescription.coaching_cues,
          reasoning_tags: [
            "full_body_region_coverage",
            region,
            ...(candidate.tags.goal_tags ?? []),
          ],
          unilateral: candidate.unilateral ?? false,
        });
      }
      if (coverageItems.length > 0) {
        const cooldownIndex = mergedBlocks.findIndex((block) => block.block_type === "cooldown");
        const coverageBlock: WorkoutBlock = {
          block_type: "accessory",
          format: "straight_sets",
          title: "Full-body coverage",
          reasoning:
            "Completes the selected full-body focus with distinct upper-body, lower-body, and core work.",
          items: coverageItems,
          estimated_minutes: coverageItems.length * 4,
        };
        if (cooldownIndex >= 0) mergedBlocks.splice(cooldownIndex, 0, coverageBlock);
        else mergedBlocks.push(coverageBlock);
      }
    }
  }
  ...
  const margin =
    req != null && req > 0 ? (req <= 30 ? 5 : req <= 45 ? 8 : 10) : 0;
  const estimated_duration_minutes =
    req != null && req > 0 ? Math.min(Math.max(sumBlockMinutes, req), req + margin) : sumBlockMinutes;
  ...
  const sessionIntentExerciseById = buildExerciseByIdForSessionIntentAnnotation(
    filtered,
    guaranteePool,
    mergedBlocks,
    sportIntentRescuePool
  );
  ensureBlockGoalIntentFallbacks(mergedBlocks, input, sessionIntentExerciseById);
  ...
  // Last-chance critical guard: do not return sessions missing required conditioning/cooldown mobility.
  if (
    fallbackValidation.violations.some(
      (v) => v.type === "conditioning_block_required" || v.type === "cooldown_mobility_required"
    )
  ) {
    const forcedBlocks = structuredClone(fallbackSession.blocks) as WorkoutBlock[];
    const forcedIds = new Set(forcedBlocks.flatMap((b) => b.items.map((i) => i.exercise_id)));
    const candidatePools = [filtered, hardFiltered, effectiveExercisePool, exercisePool];

    const pickFromPools = (pred: (e: Exercise) => boolean): Exercise | undefined => {
      for (const src of candidatePools) {
        const found = src.find((e) => {
          if (forcedIds.has(e.id)) return false;
          const shape = toConstraintEligibilityShape(e);
          if (!isExerciseAllowedByInjuries(shape, constraints)) return false;
          return pred(e);
        });
    ...
    if (
      ...
      if (cond) {
        ...
        forcedBlocks.push({
          block_type: "conditioning",
          format: "straight_sets",
          title: isTrueSteadyStateZone2Cardio(cond) ? "Zone 2" : "Conditioning",
          reasoning: "Last-chance required conditioning safeguard.",
          items: [
            {
              exercise_id: cond.id,
              exercise_name: cond.name,
              sets: p.sets,
              reps: p.reps,
              time_seconds: p.time_seconds,
              rest_seconds: p.rest_seconds,
              coaching_cues: p.coaching_cues,
              reasoning_tags: ["conditioning", "required_conditioning_block", ...(cond.tags.goal_tags ?? [])],
              unilateral: cond.unilateral ?? false,
            },
          ],
          estimated_minutes: Math.max(6, Math.round((input.duration_minutes ?? 45) * 0.2)),
        });
      }
    }

    const minCooldown = Math.max(0, constraints.min_cooldown_mobility_exercises ?? 0);
    if (minCooldown > 0) {
      let cooldown = forcedBlocks.find((b) => b.block_type === "cooldown");
      if (!cooldown) {
        cooldown = {
          block_type: "cooldown",
          format: "circuit",
          title: "Cooldown",
          reasoning: "Last-chance cooldown mobility safeguard.",
          items: [],
          estimated_minutes: 0,
        };
        forcedBlocks.push(cooldown);
      }
      const currentMobility = cooldown.items.filter((it) => {
        const ex = exercisePool.find((e) => e.id === it.exercise_id);
        return ex ? exerciseCountsAsCooldownMobilityForValidator(ex) : false;
      }).length;
      const need = Math.max(0, minCooldown - currentMobility);
      for (let i = 0; i < need; i++) {
        const ex = pickFromPools(
          (e) => isRecoveryCooldownEligible(e) && exerciseCountsAsCooldownMobilityForValidator(e)
        );
        if (!ex) break;
        forcedIds.add(ex.id);
        const p = getPrescription(
        ...
        cooldown.items.push({
          ...
          rest_seconds: 0,
          coaching_cues: p.coaching_cues,
          reasoning_tags: ["cooldown", "recovery", "required_cooldown_mobility", ...(ex.tags.goal_tags ?? [])],
          unilateral: ex.unilateral ?? false,
        });
        cooldown.estimated_minutes = (cooldown.estimated_minutes ?? 0) + 2;
      }
    }
