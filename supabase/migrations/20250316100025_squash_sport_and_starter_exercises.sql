-- Squash: add sport to catalog, sport_tag_profile, and starter_exercises tags
-- so getPreferredExerciseNamesForSportAndGoals ranks squash-relevant exercises first.

-- 1) Add squash to public.sports (Court/Field)
INSERT INTO public.sports (slug, name, category, description, is_active, popularity_tier)
VALUES ('squash', 'Squash', 'Court/Field', 'Court racquet sport with lateral movement, rotation, and anaerobic bursts.', true, 2)
...
-- 2) Add squash to sport_tag_profile
INSERT INTO public.sport_tag_profile (sport_slug, tags)
VALUES ('squash', '["sport","court","racquet","lateral","change_of_direction","anaerobic_repeats","shoulders","rotational_power","explosive","wrists"]'::jsonb)
ON CONFLICT (sport_slug) DO UPDATE SET
  tags = EXCLUDED.tags,
  updated_at = now();

-- 3) Append squash-relevant tags to starter_exercises (lateral speed, rotation, shoulder, core, work capacity)
