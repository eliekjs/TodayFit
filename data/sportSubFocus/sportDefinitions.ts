...
export const SPORT_DEFINITIONS: SportDefinition[] = [
  ...
  },
  {
    ...
    movementPatternsRanked: [
      { rank: 1, label: "Lateral edge control and single-leg bias (front/back foot load share)" },
      { rank: 2, label: "Eccentric quad absorption on variable snow and landings" },
      { rank: 3, label: "Hip abduction/ER and ankle strategy for carve and heel/toe edge" },
      { rank: 4, label: "Rotational trunk control for switch and airs" },
      { rank: 5, label: "Balance on unstable base (board as closed-chain platform)" },
    ],
    energySystems: {
      primary: "Glycolytic / anaerobic endurance (repeat runs)",
      secondary: "Aerobic base for volume days",
    },
    mustInclude: [
      "Lateral lunges / Cossack / skater landings",
      "Eccentric split squat or step-down",
      "Single-leg balance and stick landings",
      "Anti-rotation core (Pallof)",
      "Ankle and hip stability drills",
    ],
    mustAvoidOrLimit: [
      "Upper-only gym weeks",
      "Bilateral-only leg work with no lateral or single-leg exposure",
    ],
    weeklyStructureBias: [
      "2 lower strength with eccentric + lateral emphasis",
      "1 balance / landing quality",
      "1 mixed conditioning",
    ],
    engine: {
      movementPatterns: [
        { slug: "squat", rank: 1, weight: 1 },
        { slug: "rotate", rank: 2, weight: 0.92 },
        { slug: "hinge", rank: 3, weight: 0.8 },
        { slug: "locomotion", rank: 4, weight: 0.7 },
      ...
      requiredTagBoosts: [
        ...
        { tag: "balance", weight: 2.1 },
        { tag: "eccentric_strength", weight: 2 },
        { tag: "lateral_power", weight: 2 },
        { tag: "hip_stability", weight: 1.8 },
        { tag: "knee_stability", weight: 1.6 },
        { tag: "core_anti_rotation", weight: 1.7 },
        { tag: "ankle_stability", weight: 1.5 },
        { tag: "plyometric", weight: 1.5 },
      ],
      energySystemBias: {
        conditioningMinutesScale: 1.1,
        favorStimulusTags: ["anaerobic", "aerobic_zone2"],
      },
      structureBias: {
        emphasis: "strength",
        strengthShare: 0.52,
        conditioningShare: 0.33,
        hybridShare: 0.15,
        lowerBodyBias: 0.78,
        upperBodyBias: 0.12,
        fullBodyBias: 0.1,
  ...
  },
  {
    ...
    engine: {
      movementPatterns: [
        ...
        { slug: "pull", rank: 3, weight: 0.88 },
        { slug: "rotate", rank: 4, weight: 0.75 },
        { slug: "squat", rank: 5, weight: 0.7 },
      ],
      topPatterns: ["locomotion", "hinge"],
      secondaryPatterns: ["pull", "rotate"],
      requiredTagBoosts: [
        { tag: "zone2_cardio", weight: 2.2 },
        { tag: "aerobic_base", weight: 2.1 },
        { tag: "pulling_strength", weight: 2 },
        { tag: "strength_endurance", weight: 1.9 },
        { tag: "single_leg_strength", weight: 1.7 },
        { tag: "posterior_chain", weight: 1.6 },
        { tag: "core_anti_extension", weight: 1.6 },
        { tag: "lats", weight: 1.5 },
      ],
      energySystemBias: {
        conditioningMinutesScale: 1.12,
      ...
      compositionNudge: {
        conditioningPickerMinutesMultiplier: 1.15,
  ...
  },
  {
    ...
    engine: {
      movementPatterns: [
        { slug: "push", rank: 1, weight: 1 },
        { slug: "pull", rank: 2, weight: 0.95 },
        { slug: "rotate", rank: 3, weight: 0.92 },
        { slug: "squat", rank: 4, weight: 0.7 },
        { slug: "locomotion", rank: 5, weight: 0.55 },
      ],
      topPatterns: ["push", "pull"],
      secondaryPatterns: ["rotate", "squat"],
      requiredTagBoosts: [
        { tag: "shoulder_stability", weight: 2.1 },
        { tag: "scapular_control", weight: 2 },
        { tag: "strength_endurance", weight: 2 },
        { tag: "explosive_power", weight: 1.9 },
        { tag: "rotation", weight: 1.8 },
        { tag: "core_anti_rotation", weight: 1.7 },
        { tag: "balance", weight: 1.8 },
        { tag: "single_leg_strength", weight: 1.5 },
      ],
      energySystemBias: {
        conditioningMinutesScale: 1.08,
        favorStimulusTags: ["aerobic_zone2", "anaerobic"],
